...
find.groups.by.index.dynamics = function(timed_tree, 
                                         ...
                                         weight_by_time = NULL,
                                         weighting_transformation = c(NULL, 'inv_freq', 'inv_sqrt', 'inv_log','inv_sqrt_norm'),
                                         k_smooth = 5,
                                         plot_screening = F,
                                         parallelize_code = F,
                                         number_cores = 2,
  ...
  if(!is.null(weight_by_time)){
    ...
    window_weights = seq(min(metadata$time)-weight_by_time, max(metadata$time)+weight_by_time, weight_by_time)
    ...
    freqs_weighted = switch(weighting_transformation, 
                            ...
                            'inv_sqrt_norm' = {sum(sqrt(freq$counts), na.rm = T)/sqrt(freq$counts)},
                            'inv_log'  = {sum(log(freq$counts), na.rm = T)/log(freq$counts)}
    )
    if (weighting_transformation == 'inv_sqrt_norm'){
      freqs_weighted = pmin(freqs_weighted, quantile(freqs_weighted, 0.75, na.rm=TRUE))
    }

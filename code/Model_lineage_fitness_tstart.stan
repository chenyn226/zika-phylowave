// This model is modified from model in Lefrancq, Noémie, et al. "Learning the fitness dynamics of pathogens from phylogenies." Nature 637.8046 (2025): 683-690.
      ...
      for (i in 1:K){ // We deal with parent sepratelty, and make the new lienages appear 
        ...
        if(n_parents > 0){
          ...
          for (j in 1:num_elements(idx)){
            theta[idx[j], n] = log(gamma[idx[j]]) + beta[idx[j]]*(t[n] - t_start[idx[j]]);
...
data{
  ...
  vector[K] t_start_approx;       // Origin of linev ages
  ...
  vector[K] gap_max;
  array[K-GA-G] int<lower=1,upper=K> group_index;
}

parameters {
  // Proportions
  simplex[G] alpha_true_raw;   // Starting proportion of ancestral groups 
  ...
  vector<lower=0,upper=gap_max> [K] t_gap;
  vector<lower=0,upper=1> [K-G-GA] t_gap_adjust1;
         
}

transformed parameters { 
  vector[K] t_gap_adjust = rep_vector(0, K);
  t_gap_adjust[group_index] = t_gap_adjust1;
  
  vector[K] t_start = t_start_approx - t_gap;
  t_start[group_index] = t_start[group_index] - t_gap_adjust[group_index] .* t_gap[parents[group_index]];
  
  // Get alpha for each group
  vector[G] alpha_true = alpha_true_raw;
...
model {
   ...
   gamma_true ~ beta(1, 999); // Prior on starting frequencies of new groups: we expect small frequencies
   t_gap_adjust1 ~ beta(1,9);
